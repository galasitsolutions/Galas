  ...
  const getApi_Data = async () => {
    try {
      const res = await axios.get(
        "https://galasback.onrender.com/api/post/all-blogs?page=1&limit=6"  // added pagination in URL
      );
      setMy_Data(res.data);
      setIsLoading(false);
      console.log(res);
      ...
      <section className="container mx-auto px-4 flex flex-col gap-10 lg:flex-row justify-center items-stretch">
        <div className="info-box flex items-center">
          <article className="content flex flex-col items-center text-center gap-6">
            <p className="section-title p-4r">
              {" "}
              <span className="line bg-blue-800 font-semibold"></span>Our Blogs
            </p>
            <h2 id="hero-heading" className=" font-heading font-semibold  mb-4">
              {" "}
              We re{" "}
              <span style={{ color: "#2563eb" }}>
                Bridging Talented<br></br> Vision for
              </span>{" "}
              Unmatched Success
            </h2>
          </article>
                      ...
                      return (
                        <Link to={`/blog/${_id}`}>
                          <div key={_id}>
                            ...
                            <div className="p-2 ">
                              <p className="font-thin mb-2 text-gray-400 text-sm">
                                <small>{createdAt}</small>
                              </p>
                              <p>
                                <a href="" className="font-bold mb-2">
                                  {title}
                                </a>
                              </p>
                              <p className="font-thin bg-gray mt-2 text-sm lg-blogContent">
                                {content}
                              </p>
                            </div>
                          </div>
              ...
              <div className="content items-start  flex flex-col justify-start  text-start lg:w-1/2 ">
                <div className="blog-inner flex justify-end items-start gap-4 flex-col lg:flex-row">
                  <section className="features relative ">
                    <div className="feature-card flex flex-col lg:flex-row  ">
                      {" "}
                      {/* grid grid-rows-1 lg:grid-rows-3 */}
                      {isLoading && <h2>Loading...</h2>}
                      {isError && <h2>{isError}</h2>}
                      <div className=" flex flex-col ">
                        {!isLoading &&
                          blog_Data.slice(1, 4).map((post) => {
                            const { _id, imageUrl, createdAt, title, content } =
                              post;
                            return (
                              <Link to={`/blog/${_id}`}>
                                <BlogCard
