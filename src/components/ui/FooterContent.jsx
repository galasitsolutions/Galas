...
const FooterContent = ({
  ...
  return (
    <div className="footer-inner flex justify-between gap-10 p-4 flex-wrap container mx-auto px-6 flex-col lg:flex-row">
                ...
                url && (
                  <div key={key}>
                    <a href={url} title={key}>
                      <i className={`icon hover:text-blue-500`}>{key.slice(0, 2)}</i>
                    </a>
                  </div>
                )
              ))}
                ...
                {services.map((service, index) => (
                  <li key={index} className="list transition-all">
                    <div className="hover:translate-x-4 transition-transform duration-300 ease-in-out">
                      <a className="hover:text-blue-500" href="#services">{service}</a>
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="footer-section flex flex-col gap-4">
              ...
              <ul className="flex flex-col gap-2">
                {quickLinks.map((link, index) => (
                  <li key={index} className="list transition-all">
                    <div className="hover:translate-x-4 transition-transform duration-300 ease-in-out">
                      <a className="hover:text-blue-500" href={`#${link.toLowerCase()}`}>{link}</a>
                    </div>
                  </li>
                ))}
              </ul>
